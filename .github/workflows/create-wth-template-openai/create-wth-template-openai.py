...
root_path = ""
verbosityArg = False
path_to_openai_prompt_directory = ""
description_of_hack = ""
keywords = ""
openai_hack_description = ""
openai_endpoint_uri = ""
openai_api_key = ""
...
def init_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-r", "--remove_existing_directory", help="Remove existing directory", type=bool, default=True)
    ...
    parser.add_argument("-t", "--openai_api_key", help="OpenAI API key", type=str)
    parser.add_argument("-v", "--verbose", help="Verbose logging", type=bool, default=False)
    return parser
...
def create_directory_structure(delete_existing_directory):
    ...
    if verbosityArg:
        print(f"Creating {root_path}/Coach/Solutions directories...")
        
    # create the Coach & Coach/Solutions directories
    os.mkdir(f"{root_path}/Coach/Solutions")
    ...
    if verbosityArg:
        print(f"Creating {root_path}/Student/Resources directories...")
        
    # create the Student & Student/Resources directories
    os.mkdir(f"{root_path}/Student/Resources")
...
def create_challenge_and_solution(challenge_number):
    if verbosityArg:
        print(f"Creating {root_path}/Challenge-{challenge_number}.md...")
    
    challenge_response = create_challenge_markdown_file(f"{root_path}/Student", "Challenge", challenge_number)

    if verbosityArg:
        print(f"Creating {root_path}/Solution-{challenge_number}.md...")
...
def create_coach_guide_markdown_file(full_path,number_of_solutions) -> str:
    if verbosityArg:
        print(f"Creating {full_path}/README.md...")
...
def main(argv):
    ...
    root_path = f"{args.path}/{wth_directory_name}"

    path_to_template_directory = f"{args.path}/{args.template_directory_name}"
